...
4. imaptest logs in over TLS and drives `LIST`, `STATUS`, `SELECT`, `FETCH`, `STORE`, `DELETE`, `EXPUNGE`, `APPEND` and — only because `search=30` asks for it — `SEARCH`, with state tracking, quitting non-zero on the first mismatch. Its `BODY` and `TEXT` searches route through `fts_flatcurve`; because it appends from a known mbox it tracks the body words it sent, so a search that fails to return a message it should is reported as `SEARCH result missing`.
